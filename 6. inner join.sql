...
WHERE E.emp_no = EC.emp_no
    ...
    AND D.dept_nm = '개발팀' -- 개발팀 정보만 뽑기
;


-- 부서별로 총 자격등 취득 개수를 조회
SELECT
    D.dept_cd,
    D.dept_nm,
    COUNT(EC.certi_cd)
...
SELECT
    ...
    D.dept_nm
FROM tb_emp E, tb_dept D
WHERE E.addr LIKE '%용인%' -- 주소가 용인
    AND E.dept_cd = D.dept_cd
    AND E.emp_nm LIKE '김%' -- 용인사는 김씨
;
...
ON E.dept_cd = D.dept_cd -- JOIN 조건을 WHERE말고 ON으로 빼줌
WHERE E.addr LIKE '%용인%' -- 주소가 용인
    AND E.emp_nm LIKE '김%' -- 용인사는 김씨
;
...
-- 조인 조건을 안걸면 카테시안 곱이 만들어짐 (카테시안 곱 = 모든 경우의수 다 매칭)
-- 오라클 방식의 크로스 조인 (조건없이 다 매칭)
SELECT
    *
FROM test_a, test_b
;
...
FROM tb_emp A
INNER JOIN tb_dept B
USING (dept_cd) -- ON절 대체 ( ON A.dept_cd = B.dept_cd 와 같음 )
;